// ============================================================================
// TEMPLATE V2: Documento de Presupuesto — Diseño profesional
// Misma lógica de datos que PresupuestoDocumento.ts (v1).
// Layout renovado: header con caja de número destacada, sección cliente/vehículo
// en dos columnas, tabla con colores, totales resaltados, footer de validez.
// ============================================================================
...
export class PresupuestoDocumentoV2Reporte extends DocumentoBase<PresupuestoDocumentoData> {
  ...
  buildHtml(data: PresupuestoDocumentoData): string {
    ...
    const rowsHtml = leftRows.map((l, i) => {
      ...
      if (l.isHeader) {
        // Sub-encabezado de grupo — columna izquierda abarca las 3 celdas
        return `    <tr class="row-subhdr">
      <td colspan="3">${esc(l.desc)}</td>
    </tr>`;
      }
      if (r.isHeader) {
        // Sub-encabezado de grupo — columna derecha: celdas izq. y precio vacías
        return `    <tr>
      <td class="td-left"></td>
      <td class="td-price"></td>
      <td class="row-subhdr-cell">${esc(r.text)}</td>
    </tr>`;
      }
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── Variables de color ───────────────────────────── */
  /* Primario: #1B3A5C (azul marino)                    */
  /* Acento:   #2A6DB5 (azul medio — N° presupuesto)    */
  /* Fondo:    #F4F6F9 (gris muy suave)                 */
  ...
  .main-tbl th.th-left  { text-align: left;   width: 44%; }
  .main-tbl th.th-price { text-align: right;  width: 12%; }
  .main-tbl th.th-right { text-align: left;   width: 44%; }
  ...
  /* Sub-encabezado de grupo dentro de tabla */
  .row-subhdr td {
    background: #E8EEF5;
    color: #1B3A5C;
    font-weight: bold;
  ...
  /* Sub-encabezado solo en celda derecha */
  .row-subhdr-cell {
    background: #E8EEF5;
    color: #1B3A5C;
    font-weight: bold;
